...
const NAV = [
  ...
  { id: 'leads',      label: 'Leads CRM',    icon: `M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z`, badge: true },
...
const TITLES = { dashboard: 'Operational Dashboard', leads: 'Leads CRM', reports: 'Daily Reports', tasks: 'Task Board', attendance: '30-Day Attendance Grid', staff: 'Staff Management', payroll: 'Payroll Matrix', settings: 'Office Configuration' };
...
export default function AdminMatrix() {
  ...
  // Managers see Dashboard, Leads, Reports, Tasks, 30-Day Grid, Staff — not Payroll or Settings
  const visibleNav = isManager ? NAV.filter(n => ['dashboard','leads','reports','tasks','attendance','staff'].includes(n.id)) : NAV;
  ...
  return (
    <div className="h-screen bg-slate-50 flex overflow-hidden">
          ...
          {visibleNav.map((item) => {
            ...
            return (
              ...
              >
                ...
                {item.label}
                {item.badge && !active && <span className="ml-auto text-[9px] font-black bg-emerald-500 text-white px-1.5 py-0.5 rounded-full">CRM</span>}
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
        ...
        <main className={`flex-1 min-h-0 ${(activeNav === 'tasks' || activeNav === 'leads') ? 'overflow-hidden flex flex-col' : 'overflow-y-auto p-4 lg:p-8'}`}>
          {activeNav === 'tasks' ? (
            <TaskBoard />
          ) : activeNav === 'leads' ? (
            <LeadsView />
